...
|---------|-------------|------|------|
| **数据处理** | 每条数据就是一个 Record | 行级 | 一条舆情文章从拉取→清洗→打标→筛选→导出的完整处理链 |
| **代码/文件改写** | 每个代码块、每次文件改动就是一条 Record | 变更级 | 修改 `audit_filter_generic.py` 的第 45 行，新增一条规则 |
| **非结构化工作** | 每次对话就是一条 Record | 交互级 | "帮我分析比亚迪的错误模式" → Claude 的分析结论 |

**⚠️ 关键认知：这些 Record 不是互斥的，也不是互相独占的。**

在一个复杂的舆情处理任务中，Record 可能会**同时存在多种类型**，交织在同一个工作流中。审计框架必须能够同时识别和追踪所有这些 Record，而不是假设"一次只做一件事"。
...
一条舆情文章 `record_id=abc123` 经过完整 pipeline：

```jsonl
{"record_id":"abc123","step":"pull","rule":"dedup","action":"keep","reason":"hash_code 未重复"}
{"record_id":"abc123","step":"clean","rule":"Rule0_白名单","action":"keep","reason":"media_level 非白名单"}
{"record_id":"abc123","step":"clean","rule":"Rule1_词频删除","action":"keep","reason":"未触发"}
{"record_id":"abc123","step":"clean","rule":"Rule3_投诉修正","action":"modify","reason":"media_type=投诉 AND emotion=中性","before":{"news_emotion_ori":"中性"},"after":{"news_emotion_ori":"负面"}}
{"record_id":"abc123","step":"ai_classify","rule":"doubao-seed","action":"classify","reason":"tag=比亚迪品牌, confidence=92"}
{"record_id":"abc123","step":"screen","rule":"xgb_preai_v3","action":"flag","reason":"xgb_score=0.73 >= threshold=0.134"}
...
before_hash   → 修改前文件哈希
after_hash    → 修改后文件哈希
impact_scope  → 影响范围（哪些 pipeline 节点、哪些品牌、哪些规则）
...
```json
{
    ...
    "summary": "分析 123.csv 中 3 条银河M7 错误样本，归因为品牌前缀污染",
    "artifacts": [],
    "conclusions": ["AI 在 tag 中添加了品牌前缀'吉利'", "根因是 v26 prompt 缺少前缀约束示例"],
    "assumptions": []
}
```
...
│ 阶段 1：分析                                                 │
│                                                             │
│   对话 Record:  用户说"帮我分析 123.csv"                     │
│   对话 Record:  Claude 分析 3 条样本，得出"品牌前缀污染"结论  │
│   数据 Record:  读取了 123.csv 中的 3 条数据 (abc, def, ghi)  │
│                                                             │
...
│   变更 Record:  生成了 test_v27_brand_prefix_results.jsonl     │
...
class AuditContext:
    ...
    def snapshot_environment(self):
        ...
        self.environment = {
            ...
            "rules_config_hash": hash_file("audit_filter_generic.py"),
            "xgb_model_hash": hash_file("xgb_preai_v3.pkl"),
...
```jsonl
{"id":"hash_abc","batch":"batch_20260319_0600","rules":"0/0/0/0/0/0/0/0/0","disp":"ai_accepted","xgb":0.023}
```
...
action: string       # 一句话描述做了什么
                     # 例："修改 prompt 模板 v26→v27，新增品牌前缀约束"
                     # 例："分析比亚迪错误样本 522 条，归因为企业/品牌混淆"

input: string        # 输入来源
                     # 例："_0309/data_export_diff/比亚迪_diff.jsonl (522 条)"
                     # 例："docs/long_text_*.txt (v26.0)"

output: string       # 输出产物
                     # 例："docs/long_text_*.txt (v27.0), 6 处修改, +243 字符"
                     # 例："结论：企业/品牌混淆占错误的 68%"
...
action: 修改 prompt 模板 v26.0→v27.0，新增品牌前缀约束 6 处
input: docs/long_text_f5b4a468-1528-4d99-8f42-7255ebcf5438.txt (v26.0)
output: docs/long_text_f5b4a468-1528-4d99-8f42-7255ebcf5438.txt (v27.0, +243 chars)
hash_in: N/A
hash_out: N/A
```
...
action: 运行 v27 品牌前缀回归测试，10,000 条样本
input: _0309/data_export + _0309/data_export_diff (10,000 条抽样)
output: test_v27_brand_prefix_results.jsonl (10,000 条结果)
hash_in: seed=42, correct=9579, diff=421
hash_out: 回退率 18.5%, 品牌前缀错误 2 条, 无新增系统性退化
```
...
### 场景：分析比亚迪品牌前缀错误 → 修改 Prompt → 验证

```
用户：/start "修复银河M7品牌前缀错误"
Claude：会话已创建，batch_id = adhoc_20260319_1000

用户："帮我分析 123.csv 中的错误模式"
Claude：[分析...]
  3 条样本，AI 输出 tag="吉利银河M7"，列表中是「银河M7」。
  根因：品牌前缀污染。

  [AUDIT]
  batch: adhoc_20260319_1000
  action: 分析 123.csv 中 3 条银河M7 错误样本，归因为品牌前缀污染
  input: 123.csv (3 条)
  output: 结论：AI 在 tag 中添加了品牌前缀"吉利"
  hash_in: N/A
  hash_out: N/A
...
用户："跑回归测试"
Claude：[运行 test_v27_brand_prefix.py，10,000 条]

  [AUDIT]
  batch: adhoc_20260319_1000
  action: v27 回归测试 10,000 条 + v26 对照组 2,000 条
  input: _0309 数据集 (Correct=9579, Diff=421)
  output: test_v27_brand_prefix_results.jsonl
  hash_in: seed=42
  hash_out: v27 未引入退化，品牌前缀错误 2 条残留
...
| 2 | adhoc | adhoc_20260319_1000 | 修复银河M7品牌前缀错误 | ✅ 完成 | 数据:10,003 变更:4 对话:7 |
...
| prompt2 空率 | 50.0% | 47.9% | 45.2% | ⚠️ ↗ +2.1pp | batch_*.input.prompt2_empty_rate |
...
| 规则 | 触发次数 | 占比 | vs 昨日 | 来源 |
|------|---------|------|--------|------|
| R1_词频删除 | 847 | 65.2% | +12.3% | batch_*.cleaning.per_rule_summary |
| R4_4S店删除 | 56 | 4.3% | -8.2% | ... |
| R3_投诉修正 | 122 | 9.4% | +2.1% | ... |
| ... | | | | |
...
|------|-------|-------|-------|--------|------|
| 比亚迪 | 3,412 | 78 | 2.29% | -0.3pp | batch_*.ai_classify.top_errors_by_brand |
| 吉利 | 2,105 | 52 | 2.47% | +0.1pp | ... |
| ... | | | | | |
...
|---|------|---------|---------|---------|---------|
| 1 | 10:30 | docs/long_text_*.txt | v26.0→v27.0: 品牌前缀约束 | 全品牌 AI 分类 | ✅ 10k回归+对照组 |
...
| 级别 | 告警内容 | 触发批次 | 处理状态 |
|------|---------|---------|---------|
| ⚠️ WARNING | prompt2 空率连续 3 天上升 (45.8%→47.9%→50.0%) | batch_20260319_0600 | 🔴 未解决 |
...
| # | 昨日待办 | 今日状态 | 说明 |
|---|---------|---------|------|
| 1 | prompt2 空率持续上升需排查 | 🔴 未解决 | 需协调上游接口 |
| 2 | 长安 CS75PLUS 品牌前缀残留 | 🟡 已识别 | 需补充长安相关示例 |

---

## 七、明日待办

- [ ] 排查 prompt2 空率上升根因（连续 3 天）
- [ ] 补充长安品牌前缀示例到 v27 模板
...
4. 输出修正建议：
   "基于昨日日报，今日需优先处理：
    [1] prompt2 空率连续 3 天上升（CRITICAL 级别未解决）
    [2] 长安品牌前缀残留修复
...
### 🔴 CRITICAL（必须今日处理）
1. prompt2 空率连续 3 天上升 (45.8%→47.9%→50.0%)
   - 来源: 日报 §四
   - 影响: AI 筛选能力持续下降，Recall 天花板被压低
   - 建议动作: 联系上游接口负责人排查

### 🟡 WARNING（今日关注）
2. 长安品牌前缀残留（CS75PLUS, CS55PLUS）
   - 来源: Case #001 验证数据
   - 建议动作: 在 v27 模板中补充长安相关示例
...
prompt2 空率:  45.8% → 46.2% → 45.8% → 47.9% → 50.0%  ⚠️ 连续上升
...
| 信号 | 在日报中的体现 | 触发的修正动作 |
|------|--------------|-------------|
| 新品牌/新车型上线 | §二中出现未见过的品牌/车型名 | 更新可选对象列表 + 规则适配 |
| 错误模式变化 | §二品牌级错误率某品牌突然升高 | 分析新错误模式 → 更新 Prompt/规则 |
...
最近活动：
  - 昨天完成了 v27 品牌前缀修复（已验证，可上线）
  - 昨天设计了审计执行保障模块（L2 文档已完成）

待解决问题：
  - 🔴 prompt2 空率连续 3 天上升（45.8%→50.0%）— 未解决
  - 🟡 长安 CS75PLUS 品牌前缀残留 — 未修复
...
| Claude 不确定当前任务的状态 | 无法回忆"做到哪一步了" | 不知道 v27 测试已经跑过了 |
...
| "比亚迪错误率最近一周的趋势？" | 聚合最近 7 天 manifest 的 ai_classify.top_errors_by_brand | → 生成趋势表 |
| "为什么 Rule1 的触发量突然增加了？" | 对比两天的 manifest.cleaning.per_rule_summary | → 发现输入数据分布变化 |
...
| 反例（不自包含） | 正例（自包含） |
|----------------|--------------|
| action: "修改了那个文件" | action: "修改 docs/long_text_*.txt v26→v27，新增品牌前缀约束 6 处" |
| output: "结论同上" | output: "结论：品牌前缀污染是根因，AI 把'吉利'粘在'银河M7'前面" |
...
```json
{
    "action": "决定修改 Prompt v26→v27",
    "caused_by": "分析 123.csv 发现品牌前缀污染模式",
    "leads_to": "需要跑 ≥10,000 条回归测试验证",
    "depends_on": "v26 模板缺少'加前缀'示例（Case #001 根因）"
}
```
...
| **P0–P4** | Engram 长期记忆集成（详见 §11.3） | Skills + CLAUDE.md + hooks 改造 | ✅ 已完成 (v4.0.0) |
| **P2** | 实现晨间自我修正 cron（详见 §11.4） | launchd 08:03 日报 + 晨间回顾 + engram consolidate | ✅ 已完成 (v4.1.0) |
...
### 11.3 v4.0.0 Engram 长期记忆集成（2026-06-09）

将 engram（全局语义记忆层）接入 audit-harness，形成「runs/ 事实层 + engram 语义层」的双层架构。

**架构决策**：
- runs/ 是 source of truth（WORM，不可变，审计证据）
- engram 是 runs/ 的**派生语义索引**（有损，可遗忘，可 consolidate）
- 数据流向：**单向 ETL**（runs/ → engram），不回流
- engram 挂了不影响审计完整性（所有调用 try/catch）

**跨项目隔离约定**（topic 前缀）：
- `project:{name}` — 项目专属记忆
- `cross-project` — 全局共享（用户偏好、修正、红线）
- `correction` — 用户修正（salience=0.9，最高优先级恢复）

**改动清单**：

| # | 级别 | 改动 | 文件 |
|---|------|------|------|
| 1 | P0 | 移除 Stop hook 中的 `engram consolidate --json`（每次空转产生噪音） | `~/.claude/settings.json` |
| 2 | P0 | 清理 engram vault：删除 2,330 条 consolidation 日志噪音、49 个垃圾 entities、1,443 条弱 edges | `~/.engram/default.db` |
| 3 | P1 | /audit-end 新增 §7：归档 runs/ 后单向写入 engram（会话摘要、用户修正、关键结论、变更记录） | `skills/audit-end/SKILL.md` |
| 4 | P1 | user_correction 产生机制：从"指望 Agent 手写"改为"/audit-end 时统一提取" | `skills/audit-end/SKILL.md` §7.3 |
| 5 | P2 | /audit-start 新增 §2a：启动时从 engram briefing/alerts/recall 获取全局上下文 | `skills/audit-start/SKILL.md` |
| 6 | P2 | 按任务类型路由记忆源：bug fix → engram 跨项目搜；日常 → runs/ 为准 | `skills/audit-start/SKILL.md` §2a |
| 7 | P3 | /audit-recover 改为双源恢复：engram surface + runs/ audit_trail | `skills/audit-recover/SKILL.md` |
| 8 | P4 | consolidation 从 Stop hook 移至 /audit-end 和定时 cron | `~/.claude/settings.json` + `~/.claude/CLAUDE.md` |
| 9 | P4 | 全局 CLAUDE.md engram 使用规范重写（架构铁律 + 项目隔离 + 路由表） | `~/.claude/CLAUDE.md` |
...
|------|--------|--------|
| memories | 2,388 | 58 |
| procedural（噪音） | 2,330 | 0 |
| entities | 63 | 16 |
| edges | 1,517 | 74 |

### 11.4 v4.1.0 每日审计闭环自动化 + engram 安装集成（2026-06-10）

落地 §8.4/§8.5 设计的 cron 自动化，并补齐 install.sh 的 engram 配置（L2_engram §11.1 项 1、4）。

**改动清单**：

| # | 改动 | 文件 |
|---|------|------|
| 1 | 新增 `bin/audit_daily.sh`：遍历注册项目，headless claude 生成日报 + 晨间修正，再跑 engram consolidation；单项目失败不阻断 | `bin/audit_daily.sh` |
| 2 | 新增 launchd 模板（每天 08:03 触发） | `templates/com.audit-harness.daily.plist` |
| 3 | install.sh 新增 `--cron` 模式：安装 bin/ + 注册项目到 `daily_projects.conf` + 加载 launchd | `install.sh` |
| 4 | install.sh 全局安装新增 `install_engram()`：复制 engram wrapper、检测 engram CLI / Ollama 模型、幂等写入 `~/.cursor/mcp.json` 与 `~/.claude/.mcp.json`（已有配置绝不覆盖） | `install.sh` |
| 5 | 清理 v4.0.0 收尾遗留：engram vault 10 条测试残留硬删除（含 2 条 salience=0.9 假 correction）、3 个 session 状态归一为 completed | `~/.engram/default.db` + `.claude/runs/` |
...
launchd 08:03
  → bin/audit_daily.sh
      → 读 ~/.claude/audit-harness/daily_projects.conf（每行一个项目）
      → 逐项目: cd {proj} && claude -p "{日报+晨间修正 prompt}" --allowed-tools Read,Glob,Grep,Bash,Write
      → 全局一次: python3 engram/consolidate_llm.py（本地 qwen3:8b 语义提炼）
      → 日志: ~/.claude/audit-harness/logs/daily_{date}.log
```

**注册新项目**：`cd /path/to/project && bash install.sh --cron`（幂等，重复执行只更新脚本和 plist）。
...
| `~/.claude/CLAUDE.md` | 全局 Harness | 上游：全局审计规范 + engram 使用规范（v4.0.0） |
| `audit_context.py` | 代码 | ✅ 已实现：审计基础类 |
| `~/.claude/skills/audit-start/` | Skill | ✅ v4.0.0：会话启动 + 双源 context 恢复（runs/ + engram） |
| `~/.claude/skills/audit-end/` | Skill | ✅ v4.0.0：会话结束 + 审计完整性检查 + engram 写入管道 |
| `~/.claude/skills/audit-recover/` | Skill | ✅ v4.0.0：双源 context 恢复（engram surface + runs/） |
| `~/.claude/skills/audit-report-daily/` | Skill | ✅ 已实现：审计驱动日报 |
| `~/.claude/audit-harness/hooks/` | Hooks | ✅ 已实现：PostToolUse + Stop + UserPromptSubmit |
| `~/.claude/settings.json` | 配置 | ✅ v4.0.0：hooks 绑定（已移除 Stop 中的 engram consolidate） |
| `~/.engram/default.db` | 外部依赖 | engram vault（SQLite），v4.0.0 清理后 58 条真实记忆 |
| `/codes.nosync/audit-harness/` | 独立仓库 | ✅ v4.0.0：Plugin 源码 + install.sh |
